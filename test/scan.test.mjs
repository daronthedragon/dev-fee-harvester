...
const batchOf = (n) => (async function* () {
  yield Array.from({ length: n }, (_, i) => ({
    publicKey: Keypair.generate().publicKey, label: `w${i}`, secretKey: null,
  }));
})();
...
test('a wallet whose only value is a share elsewhere survives the filter', async () => {
  ...
  const enrich = async (rows) => rows.map((r, i) => (i === 2 ? {
    ...r,
    distributions: [{
      config: { address: Keypair.generate().publicKey, mint, shareholders: [] },
      mint, distributable: 2_670_512_000, userShare: 2_670_512_000, kind: 'share',
    }],
    sharingLamports: 2_670_512_000,
  } : { ...r, distributions: [] }));

  const kept = [];
  for await (const chunk of scanStream(batchOf(5), emptyChain, { workers: 0, enrich })) kept.push(...chunk);
...
test('enrich runs on every wallet, not only funded ones', async () => {
  let seen = 0;
  const enrich = async (rows) => { seen += rows.length; return rows; };
  for await (const _ of scanStream(batchOf(7), emptyChain, { workers: 0, enrich })) { /* drained */ }
...
test('a funded vault is reported with the rent floor deducted', async () => {
  const chain = {
    async getMultipleAccountsInfo(addresses) {
      ...
      return addresses.map((_, i) => (i === 0
        ? { lamports: RENT + 1_000_000_000, data: Buffer.alloc(0), owner: SYS }
        : null));
    },
  };
  const kept = [];
