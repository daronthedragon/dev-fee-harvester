...
import {
  PUMP_FEES_PROGRAM, PUMP_PROGRAM, SHARING_CONFIG_DISCRIMINATOR, SHARING_CONFIG_SIZE,
...
test('a sharing config decodes to its mint, status and shareholders', () => {
  ...
  const cfg = decodeSharingConfig(address, buildConfig(MINT, [{ address: a, bps: 7000 }, { address: b, bps: 3000 }]));
...
test('shares are split by basis points', () => {
  ...
  const cfg = decodeSharingConfig(Keypair.generate().publicKey,
    buildConfig(MINT, [{ address: me, bps: 3400 }, { address: them, bps: 6600 }]));

  assert.equal(shareFor(cfg, me, 1_000_000), 340_000);
  assert.equal(shareFor(cfg, them, 1_000_000), 660_000);
  assert.equal(shareFor(cfg, Keypair.generate().publicKey, 1_000_000), 0, 'a non-shareholder gets nothing');
});
...
test('distribute_creator_fees matches the IDL account order and appends shareholders', () => {
  ...
  const cfg = decodeSharingConfig(address, buildConfig(MINT, [{ address: a, bps: 5000 }, { address: b, bps: 5000 }]));
  const ix = distributeCreatorFeesIx(cfg);

  assert.equal(ix.programId.toBase58(), PUMP_PROGRAM.toBase58(), 'the crank lives on pump.fun, not pump_fees');
  ...
  assert.ok(ix.keys.every((k) => !k.isSigner), 'distribution is permissionless');
...
const cfgFor = (mint, holder) => decodeSharingConfig(
  sharingConfig(mint), buildConfig(mint, [{ address: holder, bps: 10000 }]));
