...
const ok = (response) => ({
  ok: true,
  status: 200,
  text: async () => JSON.stringify({ success: true, response }),
});
...
test('a failure envelope throws with the API message, even on HTTP 200', async () => {
  ...
  const { client } = clientWith(() => ({
    ok: true,
    status: 200,
    text: async () => JSON.stringify({ success: false, error: 'invalid api key' }),
  }));
  await assert.rejects(
    () => client.claimablePositions(WALLET.publicKey),
    (err) => {
...
test('a successful scan attaches the claimable total', async () => {
  const { client } = clientWith(() =>
    ok([{ baseMint: MINT.toBase58(), totalClaimableLamportsUserShare: 7_000_000 }]),
  );
  const [row] = await scanBags(client, [{ publicKey: WALLET.publicKey, label: 'w' }]);
...
test('claiming refuses a watch-only wallet', async () => {
  ...
  const results = await claimBags({}, client, [
    { publicKey: WALLET.publicKey, label: 'watch', secretKey: null },
  ]);
...
test('claiming signs and simulates what Bags returned', async () => {
  ...
  const { client } = clientWith((url) =>
    url.includes('claimable-positions')
  ...
  const results = await claimBags(
    connection,
    client,
    [{ publicKey: WALLET.publicKey, label: 'dev', secretKey: WALLET.secretKey }],
    { dryRun: true },
  );
  ...
  assert.ok(
    simulated.signatures.some((s) => s.signature !== null),
    'it was signed locally before simulating',
  );
});
