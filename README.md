...
CI runs the whole suite three ways — Ubuntu on Node 20 and 22, and Windows on Node 22 — each with all three engines installed. It fails if any test *skips*, since a browser that failed to install would otherwise pass quietly. A weekly job re-derives every on-chain constant from the deployed programs, so a program upgrade shows up as a build failure rather than as a wrong number.
